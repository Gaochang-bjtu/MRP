...
from .layerwrapper import WrappedGPT, WrappedGPT_
...
def check_sparsity(model, args):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            #W = W.to('cpu') if W.is_meta else W
            #print(W.shape)
            #print(name)
            count += (W==0).sum().item()
            ...
            sub_params += W.numel()
            #print((W==0).sum().item()/W.numel())
        #
        sparsity_ratio.append(float(sub_count)/sub_params)
        #print(f"layer {i} sparsity {float(sub_count)/sub_params:.6f}")
        
    model.config.use_cache = use_cache 
    return float(count)/total_params, sparsity_ratio
...
def check_layer_outlier_mean(mask,threshold):
    ...
    for i in range(len(mask)):
        ...
        outlier_ratio=float(count)/total_params*100
        result.append(outlier_ratio)
    return result
def compute(S):
    S_flat = S.flatten()

    # Step 2: Compute ln(ln(S)), ensuring no invalid values
    # Replace any value in S_flat <= 1 with a small positive value to avoid log domain errors
    #S_flat = torch.clamp(S_flat, min=1e-10)
    V_star = torch.norm(S_flat)
    # Step 5: Compute ϕ* = exp(cos(V*))
    phi_star = torch.exp(torch.cos(V_star))
    return round(float(phi_star), 2)
...
def prune_magnitude(args, model, sparsity_ratio,tokenizer,  device=torch.device("cuda:0"), prune_n=0, prune_m=0):
        ...
        for name in subset:
            ...
            if prune_n != 0:
                ...
                if args.sparsity_type == 'unstructured':
                    thresh = torch.sort(W_metric.flatten().cuda())[0][int(W.numel()*sparsity_ratio[i])].cpu()
                    W_mask = (W_metric<=thresh)
                    #W_mask_ = (W_metric>thresh)
                else:
                    W_norm = torch.norm(W_metric.cuda(), dim = 1, p =1)
                    thresh = torch.sort(W_norm)[0][int(W.shape[0]*sparsity_ratio[i])].cpu()
                    W_mask = (W_norm<=thresh)
                    #W_mask_ = (W_norm>thresh)
            #total_sum = torch.mean(torch.abs(W))
            #print(torch.max(W), torch.min(W),W.shape)
            
            W[W_mask] = 0
            #prune_num = torch.mean(torch.abs(W))
            #W[W_mask_] = W[W_mask_] * (total_sum / prune_num)
            #print(total_sum, prune_num, torch.mean(torch.abs(W)))
            #return 
            #print(total_sum/prune_num)
            #exit(0)
            #index = index + 1
...
def prune_wanda(args, model, sparsity_ratio, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if args.sparsity_type == "structured":
                W_norm = torch.norm(W_metric.cuda(), dim = 1, p =1)
                thresh = torch.sort(W_norm)[0][int(W_metric.shape[0]*sparsity_ratio[i])].cpu()
                W_mask = (W_norm<=thresh)
            elif prune_n != 0:
                # structured n:m sparsity
                for ii in range(W_metric.shape[1]):
                ...
                if args.use_variant:
                    ...
                    while (torch.abs(cur_sparsity - sparsity_ratio[i])>0.001) and (alpha_hist[1]-alpha_hist[0]>=0.001):
                        if cur_sparsity > sparsity_ratio[i]:
                    ...
                    indices = sort_res[1][:,:int(W_metric.shape[1]*sparsity_ratio[i])]
                    #print(indices.shape)
                    #exit(0)
...
def prune_wanda_outlier(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            print(f"pruning layer {i} name {name}")
            W_metric = torch.abs(subset[name].weight.data) * torch.sqrt(wrapped_layers[name].scaler_row.reshape((1,-1)))
            #W_metric = wrapped_layers[name].scaler_row
        ...
        #layer_wmetric = [torch.flatten(x.cpu()) for x in layer_wmetric]
        for out_ratio in [args.Hyper_m]:
            
            out_ratio_layer=check_outlier_mean(layer_wmetric,out_ratio)
            #print ("layer outlier ratio",out_ratio,out_ratio_layer)
            #for k in range(len(out_ratio_layer)):
...
@torch.no_grad()
def prune_sparsegpt(args, model,  tokenizer, dev, sparsity_ratio, prune_n=0, prune_m=0):
    ## SparseGPT code available at: https://github.com/IST-DASLab/sparsegpt/tree/f5c25005a61f96a0933ca2f95705a963585aafaa
    print('Starting ...')
    dataloader, _ = get_loaders("c4",nsamples=args.nsamples,seed=args.seed,seqlen=2048,tokenizer=tokenizer)
    ...
    for i in range(len(layers)):
        ...
        for name in gpts:
            ...
            gpts[name].fasterprune(sparsity_ratio[i], prune_n=prune_n, prune_m=prune_m, percdamp=0.01, blocksize=128)
            gpts[name].free()
            #index = index + 1
        for j in range(args.nsamples):
            if "OPT" not in model.__class__.__name__:
